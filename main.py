...
if __name__ == '__main__':
    ...
    ppr = ppr.calc_ppr_exact(A, alpha=alpha)
    print(np.around(ppr, 2))

    ps, rs = ppr.calc_ppr(G0, alpha=0.1, epsilon=0.001, nodes=nodes)
    ...
    ps_new, rs_new = ppr.DynamicSNE(G0, delta_G=delta_G0, epsilon=0.001, alpha=0.9, S=list(G0.nodes), p_pre=ps, r_pre=rs)
